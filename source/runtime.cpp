...
//longjie
static std::vector<std::filesystem::path> find_Depth3D_files(const std::vector<std::filesystem::path> &search_paths, std::initializer_list<std::filesystem::path> extensions, const std::set<std::string> &target_effects)
{
	std::error_code ec;
	std::vector<std::filesystem::path> files;
	std::vector<std::pair<std::filesystem::path, bool>> resolved_search_paths;
	...
	// Then iterate through all files in those search paths and add those with a matching extension
	const auto check_and_add_file = [&extensions, &ec, &files, &target_effects](const std::filesystem::directory_entry &entry) {
		if (!entry.is_directory(ec) &&
			std::find(extensions.begin(), extensions.end(), entry.path().extension()) != extensions.end())
		{
			if (target_effects.empty() || target_effects.find(entry.path().filename().string()) != target_effects.end())
			{
				files.emplace_back(entry); // Construct path from directory entry in-place
			}
		}
...
void reshade::runtime::WriteTo3DGlass()
{
	if (glassDeviceManager.IsGlassDevicePresent(L"VID_17EF", L"PID_B816", L"MI_03")) {
		reshade::log::message(reshade::log::level::info, "3D Glasses device found!");
		//std::vector<unsigned char> data = { 0x05, 0xAA, 0x01, 0x00 }; // 示例数据
		std::vector<unsigned char> cmdSet3D(HID_FRAME_SIZE_CMD, 0);
		cmdSet3D[00] = 0x05;
		cmdSet3D[01] = 0xaa;
		cmdSet3D[02] = 0x01;
		cmdSet3D[03] = 0x01;
		cmdSet3D[04] = 0x19;
		cmdSet3D[05] = 0x00;
		cmdSet3D[06] = 0x01;  //switch to 3D                      
		cmdSet3D[HID_FRAME_SIZE_CMD - 1] = glassDeviceManager.GetCheckSum(cmdSet3D.data(), HID_FRAME_SIZE_CMD - 1);
		glassDeviceManager.WriteToGlassDevice(cmdSet3D);

		
	}
	else {
		reshade::log::message(reshade::log::level::error, "No 3D Glasses device found!");
	}
}

void reshade::runtime::WriteTo2DGlass()
{
	if (glassDeviceManager.IsGlassDevicePresent(L"VID_17EF", L"PID_B816", L"MI_03")) {
		reshade::log::message(reshade::log::level::info, "3D Glasses device found!");
		//std::vector<unsigned char> data = { 0x05, 0xAA, 0x01, 0x00 }; // 示例数据
...
bool reshade::runtime::load_effect(const std::filesystem::path &source_file, const ini_file &preset, size_t effect_index, size_t permutation_index, bool force_load, bool preprocess_required)
{
	...
	// Generate a unique string identifying this effect 此特效唯一标识符
...
void reshade::runtime::load_effects(bool force_load_all)
{
	...
	std::set<std::string> target_effects = { "SuperDepth3D.fx" };
	const std::vector<std::filesystem::path> effect_files =
		find_Depth3D_files(_effect_search_paths, { L".fx", L".addonfx"}, target_effects);
